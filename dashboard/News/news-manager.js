...
// دالة لتحويل النص إلى HTML مع دعم الروابط
...
// دالة لاستخراج النص العادي من HTML للبحث
...
function renderTable() {
    ...
    tbody.innerHTML = paginatedNews.map(news => {
        ...
        // استخراج النص العادي لعرضه كمعاينة
        const plainContent = stripHTML(news.content);
        const previewText = plainContent.substring(0, 80) + (plainContent.length > 80 ? '...' : '');
        ...
        return `
            <tr>
                ...
                <td>
                    <div class="news-content-preview" title="${plainContent}">
                        ${previewText}
                    </div>
                    <!-- عرض المحتوى الكامل مع الروابط في نافذة منبثقة عند الضغط -->
                    <div class="news-content-full" style="display: none; max-width: 500px; background: var(--gray-100); padding: 12px; border-radius: 8px; margin-top: 8px; font-size: 0.9rem; line-height: 1.6; word-break: break-word;">
                        ${contentHTML}
                    </div>
                    <button class="btn-toggle-content" onclick="toggleContent(this)" style="background: none; border: none; color: var(--primary); cursor: pointer; font-size: 0.8rem; margin-top: 4px;">
...
// دالة لعرض/إخفاء المحتوى الكامل
window.toggleContent = function(button) {
    const row = button.closest('tr');
    const fullContent = row.querySelector('.news-content-full');
    const preview = row.querySelector('.news-content-preview');
    
    if (fullContent.style.display === 'none') {
        fullContent.style.display = 'block';
        preview.style.display = 'none';
        button.innerHTML = '<i class="fa-solid fa-chevron-up"></i> عرض أقل';
    } else {
        fullContent.style.display = 'none';
        preview.style.display = 'block';
        button.innerHTML = '<i class="fa-solid fa-chevron-down"></i> عرض المزيد';
    }
};
